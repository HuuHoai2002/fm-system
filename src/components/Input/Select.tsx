...
const Select = React.forwardRef<HTMLInputElement, SelectProps>((props, ref) => {
  ...
  React.useEffect(() => {
    if (defaultValue) {
      ...
      const defaultSelectIndex = options.findIndex(
        (o) => o.value === defaultValue
      );

      setIsSelected(options[defaultSelectIndex]);

      if (onChange) {
        onChange(options[defaultSelectIndex]);
      }
    } else {
      ...
      />
      {
        <div>
          {isFocused && (
            <div
              className={classNames(
                "absolute w-full z-[99999] bg-white rounded-md border-t border-l border-r border-gray-200 shadow-sm overflow-y-scroll scrollbar-select transition-all duration-200 max-h-40 mt-[1px]"
              )}
            >
              {options.map((option) => (
                <div
